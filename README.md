...
**Status: Phase 6 complete.** A deterministic supervisory controller runs in closed loop and
...
repairs a model EnergyPlus will not run. Every figure below is regenerated by
`make evidence` and published as [docs/evidence/report.html](docs/evidence/report.html).
...
| `make evidence` | Regenerate every published number, and the report |
...
| foresight bound | 731,092 (−4.80%) | 315.1 (−8.66%) | 50.0 | 505 | 1,015 (−1.4%) |
| agent, model-supervised | 756,427 (−1.50%) | 340.4 (−1.31%) | 55.0 | 509 | 1,033 (+0.3%) |

`foresight` is not a controller worth shipping. It is the same reset loop driven by the day's
forecast peak instead of the current reading, on the same degraded forecast the model gets:
what perfect discrimination about the day ahead would be worth. Over a single season it lands
on the reactive controller to within 0.03 points. Over a year it saves 0.5 points less and is
22 K·h more comfortable — a different point on the same frontier, not a better one.
...
anticipating the shoulder-season afternoon moves along that frontier rather than past it. And run unchanged on New
...
```
ecoloop/
  evidence.py    regenerate every published number and render the report
...
Runs the full ladder across both climates and all four periods, commissions all three
building-and-climate pairs, and writes [docs/evidence/evidence.json](docs/evidence/evidence.json)
and [docs/evidence/report.html](docs/evidence/report.html). Nothing in the report is
transcribed — it is rendered from the same objects the simulations returned, so a figure in
this file and a figure in the repository cannot disagree.

The agent's decision journal is committed under `docs/evidence/decisions/`, 371 records of
what the model was asked and what it answered. A clean clone replays the agent arm from it
exactly, in seconds, **without Ollama installed**. Pull the model only to generate new
decisions rather than replay recorded ones.

`tests/test_evidence.py` reads the committed evidence back and asserts the claims this README
makes about it, so the two cannot drift apart silently.
